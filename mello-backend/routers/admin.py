...
async def get_pending_counselors(
    ...
    pending_counselors = db.query(User).filter(
        and_(User.role == UserRole.COUNSELOR, User.counselor_status == CounselorStatus.PENDING)
    ).all()
...
async def get_mood_analytics(
    ...
    avg_mood = db.query(func.avg(MoodEntry.mood_score)).filter(MoodEntry.date >= start_date).scalar()
...
async def get_appointment_analytics(
    ...
    counselor_stats = db.query(
        User.name,
        User.specialization,
        func.count(Booking.id).label('appointment_count')
    ).join(
        Booking, User.id == Booking.counselor_id
    ).filter(
        User.role == UserRole.COUNSELOR
    ).group_by(User.id).all()
    
    # Assessment-based counselor distribution
    assessment_counselor_match = db.query(
    ...
    return {
        ...
        "severity_distribution": {severity: count for severity, count in assessment_counselor_match}
    }
...
async def get_chat_analytics(
    ...
    # Most common issues
    total_chats = db.query(ChatbotLog).filter(ChatbotLog.timestamp >= start_date).count()
    
    return {
        "total_chats": total_chats,
        "category_breakdown": {category: count for category, count in category_breakdown},
        "daily_volume": [
...
async def create_newsletter(
    ...
    return {"message": "Newsletter created successfully", "newsletter_id": new_newsletter.id}
...
async def get_newsletters(
    ...
    return [{
        ...
        "author_name": db.query(User).filter(User.id == newsletter.author_id).first().name
...
async def publish_newsletter(
    ...
    db.commit()
    return {"message": f"Newsletter {'published' if newsletter.is_published else 'unpublished'} successfully"}
...
async def get_counselor_reports(
    ...
    reports = db.query(CounselorReport).filter(
        CounselorReport.status == "submitted"
    ).all()
