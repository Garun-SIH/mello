...
from models import User, Booking, ChatbotLog, MoodEntry, CounselorReport, Feedback
...
async def get_patient_analytics(
    ...
    patient_ids = db.query(Booking.user_id).filter(
    ...
    mood_entries = db.query(MoodEntry).filter(
        ...
        MoodEntry.date >= datetime.now() - timedelta(days=30)
    ).all()
    ...
    chat_activity = db.query(
        ...
        ChatbotLog.user_id.in_(patient_ids)
    ).filter(
        ChatbotLog.timestamp >= datetime.now() - timedelta(days=30)
...
async def get_patient_mood_trends(
    ...
    daily_mood = db.query(
        ...
        func.date(MoodEntry.date)
    ).order_by(func.date(MoodEntry.date)).all()
...
async def create_counselor_report(
    ...
    if not booking_exists:
        raise HTTPException(status_code=403, detail="Patient not assigned to this counselor")
...
async def get_counselor_reports(
    ...
    query = db.query(CounselorReport).filter(CounselorReport.counselor_id == counselor.id)
...
async def get_counselor_schedule(
    ...
    appointments = db.query(Booking).filter(
        and_(
            Booking.counselor_id == counselor.id,
    ...
    ).order_by(Booking.preferred_datetime).all()
    ...
    return [{
        ...
        "issue_type": booking.issue_description[:50] + "..." if len(booking.issue_description) > 50 else booking.issue_description
    } for booking in appointments]
...
async def get_counselor_patients(
    ...
    for (patient_id,) in patient_ids:
        ...
        if patient:
            ...
            ).order_by(Booking.preferred_datetime.desc()).first()
            ...
            patients.append({
                ...
                "last_session": latest_booking.preferred_datetime if latest_booking else None,
                "total_sessions": total_sessions,
                "status": latest_booking.status if latest_booking else "no_sessions"
            })
...
async def get_patient_history(
    ...
    ).order_by(Booking.preferred_datetime.desc()).all()
    ...
    ).order_by(MoodEntry.date.desc()).limit(10).all()
    
    # Get counselor reports for this patient
    reports = db.query(CounselorReport).filter(
        and_(
            CounselorReport.patient_id == patient_id,
            CounselorReport.counselor_id == counselor.id
        )
    ).order_by(CounselorReport.session_date.desc()).all()
...
async def update_counselor_report(
    ...
    report = db.query(CounselorReport).filter(
        and_(
...
async def get_counselor_dashboard_stats(
    ...
    # Total patients
    total_patients = db.query(Booking.user_id).filter(
        Booking.counselor_id == counselor.id
    ).distinct().count()
    ...
    # Pending reports
    pending_reports = db.query(CounselorReport).filter(
        and_(
            CounselorReport.counselor_id == counselor.id,
