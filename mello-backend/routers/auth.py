...
from models import CounselorStatus, User, UserRole
...
async def register_counselor(
    counselor_data: CounselorRegistration, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        existing_user = (
            db.query(User)
            .filter(User.firebase_uid == counselor_data.firebase_uid)
            .first()
        )
        if existing_user:
            raise HTTPException(status_code=400, detail="User already registered")
        ...
        return {
            "message": "Counselor registration submitted for approval",
            "user_id": new_counselor.id,
        }
...
async def register_admin(
    ...
    try:
        # Check if user already exists
        existing_user = (
            db.query(User).filter(User.firebase_uid == admin_data.firebase_uid).first()
        )
        if existing_user:
            raise HTTPException(status_code=400, detail="User already registered")
...
async def get_profile(current_user: User = Depends(get_current_user)):
    ...
    return {
        ...
        "counselor_status": current_user.counselor_status.value
        if current_user.counselor_status
        else None,
