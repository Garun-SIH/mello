...
def categorize_message(message: str) -> str:
    ...
    if any(word in message_lower for word in ['stress', 'stressed', 'pressure', 'overwhelm']):
        return 'stress'
    elif any(word in message_lower for word in ['sleep', 'insomnia', 'tired', 'exhausted']):
        return 'sleep'
    elif any(word in message_lower for word in ['anxiety', 'anxious', 'worry', 'nervous', 'exam']):
        return 'anxiety'
...
def should_escalate(message: str, response: str) -> bool:
    ...
    escalation_keywords = [
        ...
        'severe depression', 'can\'t cope', 'emergency', 'crisis'
    ]
...
async def chat_with_bot(chat_message: ChatMessage, db: Session = Depends(get_db)):
    try:
        ...
        if chat_message.student_id:
            ...
            user = db.query(User).filter(User.firebase_uid == chat_message.student_id).first()
            if not user:
                user = User(
...
async def get_chat_history(student_id: str, db: Session = Depends(get_db)):
    ...
    logs = db.query(ChatbotLog).filter(
        ChatbotLog.user_id == user.id
    ).order_by(ChatbotLog.timestamp.desc()).limit(20).all()
