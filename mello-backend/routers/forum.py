...
from models import User, ForumPost, ForumReply
...
async def get_posts(
    ...
    query = db.query(ForumPost).filter(
        and_(
            ForumPost.is_moderated == True,
            ForumPost.moderation_action != "removed"
        )
    )
    ...
    return [{
        ...
        "author_name": "Anonymous" if post.is_anonymous else db.query(User).filter(User.id == post.user_id).first().name,
...
async def get_post_with_replies(
    ...
    replies_data = [{
        ...
        "author_name": "Anonymous" if reply.is_anonymous else db.query(User).filter(User.id == reply.user_id).first().name,
