...
from models import User, MoodEntry, Feedback, Newsletter, Booking, Assessment, ChatbotLog
...
async def create_mood_entry(
    ...
    existing_entry = db.query(MoodEntry).filter(
        and_(
            ...
            func.date(MoodEntry.date) == today
        )
    ).first()
...
async def get_mood_history(
    ...
    mood_entries = db.query(MoodEntry).filter(
        MoodEntry.user_id == current_user.id
    ).order_by(desc(MoodEntry.date)).limit(days).all()
...
async def get_personal_mood_analytics(
    ...
    if mid_point > 0:
        recent_avg_mood = sum(entry.mood_score for entry in mood_entries[:mid_point]) / mid_point
        older_avg_mood = sum(entry.mood_score for entry in mood_entries[mid_point:]) / (len(mood_entries) - mid_point)
        mood_trend = "improving" if recent_avg_mood > older_avg_mood else "declining" if recent_avg_mood < older_avg_mood else "stable"
    else:
        mood_trend = "stable"
    ...
    if len(sleep_entries) > 5:
        ...
        good_sleep_moods = [entry.mood_score for entry in sleep_entries if 7 <= entry.sleep_hours <= 9]
        poor_sleep_moods = [entry.mood_score for entry in sleep_entries if entry.sleep_hours < 6 or entry.sleep_hours > 10]
        ...
        if good_sleep_moods and poor_sleep_moods:
            ...
            sleep_mood_correlation = "positive" if good_sleep_avg > poor_sleep_avg else "negative"
...
async def get_user_dashboard(
    ...
    ).order_by(Booking.preferred_datetime).limit(3).all()
    
    # Recent assessments
    recent_assessments = db.query(Assessment).filter(
        Assessment.user_id == current_user.id
    ).order_by(desc(Assessment.created_at)).limit(3).all()
    
    # Chat activity this week
    week_start = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
    week_start = week_start - timedelta(days=week_start.weekday())
    
    weekly_chats = db.query(func.count(ChatbotLog.id)).filter(
        and_(
    ...
    # Mood streak
    mood_streak = db.query(func.count(MoodEntry.id)).filter(
        MoodEntry.user_id == current_user.id
    ).scalar()
    ...
    return {
        ...
        "upcoming_appointments": [{
            ...
            "counselor_name": db.query(User).filter(User.id == booking.counselor_id).first().name,
