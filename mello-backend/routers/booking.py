...
from models import Booking, CounselorStatus, User, UserRole
...
async def create_booking(
    ...
    try:
        ...
        if not counselor:
            raise HTTPException(
                status_code=404, detail="Counselor not found or not approved"
            )
...
async def update_booking_status(
    ...
    if current_user.role == UserRole.USER and booking.user_id != current_user.id:
        ...
        and booking.counselor_id != current_user.id
    ):
        raise HTTPException(
            status_code=403, detail="Not authorized to update this booking"
        )
